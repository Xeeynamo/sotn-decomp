...
// NOTE: This function takes no args, but one of the callers gives it a 0.
// To prevent the compiler from exploding, we leave args empty, not void.
void func_8012CA64() {
    s32 anim = 0xDE;
        ...
        if (PLAYER.pose >= 3) {
            if (PLAYER.poseTimer < 0) {
                ...
                if (!(directionsPressed & (PAD_LEFT | PAD_RIGHT))) {
                    // Function takes no args but we give one anyway
                    func_8012CA64(0);
                    break;
                }
            }
