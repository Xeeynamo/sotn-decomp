...
void ClearBackbuffer(void) { ClearImage(&g_Vram[4], 0, 0, 0); }
...
void HandleGameOver(void) {
    ...
    case Gameover_4:
        if (g_UseDisk) {
            ...
            LoadImage(&g_Vram[10], (u_long*)STAGE_PRG_PTR);
            LoadImage(&g_Vram[11], (u_long*)STAGE_PRG_PTR + 0x2000);
            LoadImage(&g_Vram[7], (u_long*)STAGE_PRG_PTR + 0x6000);
            StoreImage(&g_Vram[7], (u_long*)g_Clut[2]);
            LoadImage(&g_Vram[5], (u_long*)STAGE_PRG_PTR + 0x6800);
            StoreImage(&g_Vram[5], (u_long*)g_Clut[0]);
        } else {
...
void HandleEnding(void) {
    ...
    case 1:
        if (g_UseDisk) {
            ...
            LoadImage(&g_Vram[12], (u_long*)STAGE_PRG_PTR);
            LoadImage(&g_Vram[11], (u_long*)STAGE_PRG_PTR + 0x800);
            LoadImage(&g_Vram[7], (u_long*)STAGE_PRG_PTR + 0x4800);
            StoreImage(&g_Vram[7], (u_long*)g_Clut[2]);
            LoadImage(&g_Vram[5], (u_long*)STAGE_PRG_PTR + 0x5000);
            StoreImage(&g_Vram[5], (u_long*)g_Clut[0]);
        } else {
