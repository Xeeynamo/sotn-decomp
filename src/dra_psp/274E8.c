...
extern u16 D_psp_091634C0[];
extern u16 D_psp_091654C0[];
extern char D_psp_09160BB8[];
    ...
    for (j = 0, i = 0, gfxLoad = g_GfxLoad; i < 0x10; i++) {
        ...
        case 4:
            ...
            for (; j < 4; j++) {
                ...
                if (over) {
                    psp_sprintf(buf, D_psp_09160BB8, cmp, over);
                    DebugInputWait(buf);
                }
    ...
    if (D_80137538[index] != palette) {
        for (i = 0; i < 16; i++) {
            g_Clut[index * 0x10 + i + 0x1D00] =
                D_psp_091634C0[palette * 0x10 + i];
        }
        x = index & 0xF;
        y = index / 16;
        if (palette < 0x100) {
            ptr = &D_psp_091634C0[palette * 0x10];
        } else {
