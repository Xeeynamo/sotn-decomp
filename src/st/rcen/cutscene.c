...
#ifndef VERSION_PSP
// PSP has this zeroed out in BSS
static u8 v_coords[] = {0, 0};
#endif

// These cluts are based on the actor name indexing
static u16 cluts[] = {PAL_PORTRAIT_ALUCARD, PAL_PORTRAIT_SHAFT};
...
#ifdef VERSION_PSP
static u8 actor_name_len_index[] = {NAME_IDX_ALUCARD, NAME_IDX_SHAFT};
// This is passed to, but seems to be unused within DrawCutsceneActorName
static u8 actor_name_index[] = {NAME_IDX_ALUCARD, NAME_IDX_SHAFT};
...
void OVL_EXPORT(EntityCutscene)(Entity* self) {
    ...
    if (self->step && dialogue_started && !OVL_EXPORT(SkipCutscene)) {
        if (g_GameClearFlag) {
            CutsceneSkip(self);
        } else if (self->params) {
            if (g_Settings.D_8003CB04 & 0x100000) {
                CutsceneSkip(self);
            }
        } else if (g_Settings.D_8003CB04 & 0x80000) {
            CutsceneSkip(self);
        }
    }
        ...
        while (true) {
            ...
            nextChar = *OVL_EXPORT(Dialogue).scriptCur++;
...
#else
                if (OVL_EXPORT(Dialogue).nextCharY > CS_LINE_MAX) {
                ...
                if (j & 0x80) {
                    ...
                    OVL_EXPORT(Dialogue).clutIndex = cutscene_unk_4_cluts[i];
                    CutsceneUnk1();
                    CutsceneUnk4();
                    prim->priority = 510;
...
// psp and psx refuse to agree on this particular increment
                ...
                case CSOP_SCRIPT_UNKNOWN_11:
                    ...
                    *OVL_EXPORT(Dialogue).scriptCur--;
                    return;
                case CSOP_SET_END:
                    ptr = (u32)*OVL_EXPORT(Dialogue).scriptCur++;
                    ptr <<= 4;
                    ptr |= (u32)*OVL_EXPORT(Dialogue).scriptCur++;
                    ptr <<= 4;
                    ptr |= (u32)*OVL_EXPORT(Dialogue).scriptCur++;
        ...
        for (i = 0; i < 8; i++) {
            // lots of byte shuffling here
            charBuffer2 =
                &cutsceneUnk3Unknown[((OVL_EXPORT(Dialogue).nextCharX -
                                       OVL_EXPORT(Dialogue).nextLineX) /
                                      2) +
                                     (i * 112)];
    ...
    case DIALOGUE_END:
        if (self->params) {
            g_Settings.D_8003CB04 |= 0x100000;
        } else {
            g_Settings.D_8003CB04 |= 0x80000;
        }
