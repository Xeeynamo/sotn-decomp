...
void EntityJackOBones(Entity* self) {
    ...
    case JACKO_4:                                         
        ...
        if (!var_s2){
            ...
            self->ext.ILLEGAL.u8[0] = D_pspeu_092590F8[self->params & 1][var_s2];
            break;
        }
        if ((var_s2 & 0x80) && (self->animCurFrame == 0xB)) {
            other = AllocEntity(&g_Entities[160], &g_Entities[192]);
            if (other != NULL) {
                PlaySfxPositional(0x6C8);
                CreateEntityFromCurrentEntity(0x26, other);
                if (self->params) {
                    xShift = -0x10;
                } else {
                    xShift = 8;
    ...
    case 6:                                         
        ...
        for(i = 0; i < 6; i++){
            ...
            CreateEntityFromCurrentEntity(0x25, other);
            other->facingLeft = self->facingLeft;
            other->params = i;
