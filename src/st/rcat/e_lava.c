...
// This file shares much of its design with the lava/embers from CAT, including
...
// Params 0 = Skeleton/Frozen Half rooms, non-transparent, spawns snowflakes
// Params non-0 = Bridge room, transparent, no snowflakes
void EntityLava(Entity* self) {
    ...
    case 0:
        ...
        // Top layer of the liquid sits behind the Bridge platform
...
static void FadeOutSnowflake(Primitive* prim) {
    if (g_Timer % prim->u3) {
        return;
    }
    prim->y0++;

    if (prim->y0 > prim->u2 + 112) {
        prim->r0 -= 2;
        prim->g0 -= 2;
        prim->b0 -= 7;
    }

    // Once we hit a color threshold, the snowflake has "melted"
    // and is hidden
    if (prim->b0 < 8) {
        prim->p3 = 0;
        prim->drawMode = DRAW_HIDE;
