...
static void func_pspeu_09255F98(FrozenShadePrim* prim) {
    s16 dx, dy;
...
extern u8 D_pspeu_0929F5B8[];
extern u8 D_pspeu_0929F5C8[];
extern u8 D_pspeu_0929F5D8[];
extern u8 D_pspeu_0929F5E0[];
extern u8 D_pspeu_0929F5F8[];
extern s32 D_pspeu_0929F610[];
extern s32 D_pspeu_0929F630[];
extern u8 D_pspeu_0929F650[];
extern s16 D_pspeu_0929F680[][3];
extern u8 D_pspeu_0929F6F0[];
extern s16 D_pspeu_0929F730[];
extern s16 D_pspeu_0929F790[];
extern s16 D_pspeu_0929F798[];
...
void func_pspeu_09256330(Entity* self) {
    ...
    if (self->ext.et_801D96FC.unk82) {
        ...
        while (prim != NULL) {
            if (!prim->v2) {
                if (!prim->p3) {
                    if (self->facingLeft) {
                        prim->posX.val += D_pspeu_0929F610[prim->u2];
                    } else {
                        prim->posX.val -= D_pspeu_0929F610[prim->u2];
                    }
                    prim->posY.val +=
                        D_pspeu_0929F630[prim->u2] - (prim->u3++ << 10);
                    func_pspeu_09255F98(prim);
                    PRED(prim)--;
                    PGRN(prim)--;
                    PBLU(prim) -= 3;
                    if (prim->p2) {
                ...
                if (!prim->v2) {
                    ...
                    prim->u2 = D_pspeu_0929F650[prim->p1];
                    prim->p1 &= 3;
                    prim->u3 = 0;
                    prim->p2 = 0;
                    prim->r0 = 0x40;
    ...
    switch (self->step) {
    case 0:
        InitializeEntity(D_pspeu_09267BA0);
        self->unk6C = 0x40;
    ...
    case 1:
        AnimateEntity(D_pspeu_0929F5B8, self);
        ...
        func_pspeu_09256138(self);
        break;

    case 2:
        AnimateEntity(D_pspeu_0929F5F8, self);
        tempVar = GetDistanceToPlayerX();
        if (tempVar > 0x10) {
    ...
    case 3:
        ...
        case 0:
            ...
            tempVar = AnimateEntity(D_pspeu_0929F5C8, self);
            ...
            var_s4 = 0;
            iptr = D_pspeu_0929F6F0;
            point = D_pspeu_0929F730;
            prim = (FrozenShadePrim*)&g_PrimBuf[self->primIndex];
            while (prim != NULL) {
                for (i = 0; i < 4; i++) {
                    s16* temp = D_pspeu_0929F680[*iptr];
                    points[i]->vx = *temp++;
                    points[i]->vy = *temp++;
                    points[i]->vz = *temp;
                    iptr++;
        ...
        case 1:
            AnimateEntity(D_pspeu_0929F5D8, self);
            if (self->velocityX != 0) {
                if (self->facingLeft) {
        ...
        case 2:
            if (!AnimateEntity(D_pspeu_0929F5E0, self)) {
                SetStep(1);
            ...
            if (++self->ext.et_801D96FC.unk8A == 1) {
                ...
                for (i = 0; i < 3; i++) {
                    ...
                    if (tempEntity != NULL) {
                        ...
                        tempEntity->posX.i.hi += D_pspeu_0929F790[tempVar];
                        tempEntity->posY.i.hi += D_pspeu_0929F798[tempVar];
                        tempEntity->params = i;
...
INCLUDE_ASM("st/no4_psp/psp/no4_psp/unk_1E818", func_pspeu_09258910);

INCLUDE_ASM("st/no4_psp/psp/no4_psp/unk_1E818", func_pspeu_09258960);

INCLUDE_ASM("st/no4_psp/psp/no4_psp/unk_1E818", func_pspeu_09258E30);

INCLUDE_ASM("st/no4_psp/psp/no4_psp/unk_1E818", func_pspeu_09259648);
