...
void EntityMermanRockLeftSide(Entity* self) {
    ...
    case 1:
        ...
        if (self->ext.mermanRock.unk84 > 1) {
            ...
            g_CastleFlags[JEWEL_ROOM_STEPS] |= rockBroken;
            self->hitboxState = 1;
            self->step++;
        }
        break;
    ...
    case 2:
        if ((self->hitFlags) && (g_Player.status & PLAYER_STATUS_WOLF_FORM)) {
            g_CastleFlags[JEWEL_ROOM_STEPS] |= wolfFlag;
        }
        break;
    }
}
...
void EntityMermanRockRightSide(Entity* self) {
    ...
    case 2:
        if ((self->hitFlags) && (g_Player.status & PLAYER_STATUS_BAT_FORM)) {
            g_CastleFlags[JEWEL_ROOM_STEPS] |= batFlag;
        }
        break;
    }
}
...
void EntityJewelSwordDoor(Entity* self) {
    ...
    case 0:
        InitializeEntity(g_EInitInteractable);
        if (g_CastleFlags[JEWEL_ROOM_OPEN]) {
            self->step = 2;
        }
        break;

    case 1:
        if ((g_CastleFlags[JEWEL_ROOM_STEPS] & 4) &&
            (g_CastleFlags[JEWEL_ROOM_STEPS] & 8)) {
    ...
    case 2:
        ...
        g_CastleFlags[JEWEL_ROOM_OPEN] |= 1;
        g_api.RevealSecretPassageAtPlayerPositionOnMap(JEWEL_ROOM_OPEN);
        self->step++;
        break;
    }
}
