...
// BSS
u32 OVL_EXPORT(CutsceneFlags); // used by EntityMariaNZ0
s32 OVL_EXPORT(SkipCutscene);  // used by EntityMariaNZ0
static Dialogue OVL_EXPORT(Dialogue);
static u8 D_pspeu_0928BDF0;
static s32 D_pspeu_0928BDD0[8];
static u8 cutsceneUnk3Unknown[0x380];
static u16 actor_names[0x100];

static u8 D_801813C8[2] = {0x00, 0x40};
static u8 D_801813CC[2];
...
static u8 D_pspeu_09289C38[] = {3, 4, 9, 7};
static u8 D_pspeu_09289C40[] = {3, 4, 9, 7};

extern u_long* D_pspeu_092759C8;
extern u_long* D_pspeu_09276748;
...
static s32 OVL_EXPORT(IsCutsceneDone); // BSS
...
void OVL_EXPORT(EntityCutscene)(Entity* self) {
    ...
    case 1:
            ...
            if (!(nextChar & 0x80)) {
                ...
                case CSOP_LINE_BREAK:
                    ...
                    if (!(OVL_EXPORT(Dialogue).unk12 & 1)) {
                        OVL_EXPORT(Dialogue).nextLineY += 16;
                    }
                    OVL_EXPORT(Dialogue).nextCharY++;
                    OVL_EXPORT(Dialogue).nextCharY &= 3;
                    CutsceneUnk4();
                    if (!(OVL_EXPORT(Dialogue).unk12 & 1)) {
                        if (OVL_EXPORT(Dialogue).nextCharY > 2) {
                ...
                case CSOP_SET_PORTRAIT:
                    ...
                    DrawCutsceneActorName(
                        i, self, actor_names, D_pspeu_09289C38,
                        D_pspeu_09289C40, 4);
                ...
                case CSOP_LOAD_PORTRAIT:
                    if (OVL_EXPORT(SkipCutscene)) {
                        ...
                        switch (ptr) {
                        case 0:
                            ptr = (u_long)&D_pspeu_09276748;
                            break;
                        case 1:
                            ptr = (u_long)&D_pspeu_092759C8;
                            break;
                        case 2:
        ...
        for (i = 0; i < 8; i++) {
            // lots of byte shuffling here
            var_s1 = &cutsceneUnk3Unknown[((OVL_EXPORT(Dialogue).nextCharX -
                                            OVL_EXPORT(Dialogue).nextLineX) /
                                           2) +
                                          (i * 112)];
        ...
        LoadTPage((u_long*)cutsceneUnk3Unknown, 0, 0,
                  OVL_EXPORT(Dialogue).nextLineX, vCoord, 0xE0, 8);
        if (nextChar != 0x20) {
        ...
        D_pspeu_0928BDF0 = nextChar;
        break;
    case 2:
        // very similar to ScaleCutsceneAvatar, but inlined
        ptr = (OVL_EXPORT(Dialogue).nextCharY + 1) & 3;
        if (!self->step_s) {
