...
INCLUDE_ASM("st/nz0/psx/nz0/33FCC", EntityCloseBossRoom);
...
#ifndef NON_MATCHING
INCLUDE_ASM("st/nz0/psx/nz0/33FCC", EntityGaibon);
#else
void EntityGaibon(Entity* self) {
    ...
    if (!(self->flags & FLAG_DEAD)) {
        ...
        if (self->velocityX > 0 && slograGaibonDistX > 784) {
            self->posX.i.hi = 784 - g_Tilemap.scrollX.i.hi;
        }

        if (self->velocityX < 0 && slograGaibonDistX < 96) {
            self->posX.i.hi = 96 - g_Tilemap.scrollX.i.hi;
        }
        if (self->velocityY > 0 && slograGaibonDistY > 420) {
            self->posY.i.hi = 420 - g_Tilemap.scrollY.i.hi;
        }
