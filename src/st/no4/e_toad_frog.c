...
s32 func_us_801D7FAC(s32 index) {
    Collider collider;
    s32 posX;
    s32 posY;

    switch (g_CurrentEntity->ext.toad.step) {
    case 0:
        if (!AnimateEntity(D_us_801826C0, g_CurrentEntity)) {
            ...
            g_CurrentEntity->ext.toad.step++;
            PlaySfxPositional(SFX_BLIPS_C);
            g_CurrentEntity->ext.toad.unk82 = 4;
        }
        break;
    case 1:
        ...
        if (UnkCollisionFunc3(D_us_801826C8) & 1) {
            ...
            g_CurrentEntity->ext.toad.step = 2;
        }
        ...
        if (g_CurrentEntity->ext.toad.unk82) {
            g_CurrentEntity->animCurFrame = 0x1B;
            g_CurrentEntity->ext.toad.unk82--;
        }
        break;
    case 2:
        if (!AnimateEntity(D_us_801826C0, g_CurrentEntity)) {
...
INCLUDE_ASM("st/no4/nonmatchings/e_toad_frog", func_us_801D81B4);
