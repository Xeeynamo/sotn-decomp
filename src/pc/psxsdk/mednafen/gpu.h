...
// WARNING WARNING WARNING:  ONLY use CanRead() method of BlitterFIFO, and NOT
// CanWrite(), since the FIFO is larger than the actual PS1 GPU FIFO to
// accommodate our lack of fancy superscalarish command sequencer.
...
struct PS_GPU {
    ...
    uint32 CLUT_Cache_VB; // Don't try to be clever and reduce it to 16 bits...
                          // ~0U is value for invalidated state.

    struct // Speedup-cache variables, derived from other variables; shouldn't
           // be saved in save states.
    {
    ...
    FastFIFO<uint32, 0x20> BlitterFIFO; // 0x10 on actual PS1 GPU, 0x20 here(see
                                        // comment at top of gpu.h)
    ...
    //
    //
    // Powers of 2 for faster multiple equality testing(just for multi-testing;
    // InCmd itself will only contain 0, or a power of 2).
    enum {
        INCMD_NONE = 0,
        INCMD_PLINE = (1 << 0),
        INCMD_QUAD = (1 << 1),
    ...
    uint8 DitherLUT[4][4]
                   [512]; // Y, X, 8-bit source value(256 extra for saturation)
...
MDFN_HIDE extern PS_GPU GPU;

extern "C" void GPU_Init(bool pal_clock_and_tv) MDFN_COLD;
void GPU_Kill(void) MDFN_COLD;

void GPU_SetGetVideoParams(MDFNGI* gi, const bool caspect, const int sls,
                           const int sle, const bool show_h_overscan) MDFN_COLD;

void GPU_GetGunXTranslation(float* scale, float* offs);

extern "C" void GPU_Power(void) MDFN_COLD;

// void GPU_StateAction(StateMem *sm, const unsigned load, const bool
// data_only);

void GPU_ResetTS(void);

extern "C" void GPU_StartFrame(EmulateSpecStruct* espec);

extern "C" MDFN_FASTCALL pscpu_timestamp_t GPU_Update(
    const pscpu_timestamp_t timestamp);

MDFN_FASTCALL extern "C" void GPU_Write(
    const pscpu_timestamp_t timestamp, uint32 A, uint32 V);
...
static INLINE bool GPU_CalcFIFOReadyBit(void) {
    ...
    if (GPU.BlitterFIFO.CanRead() >=
        GPU.Commands[GPU.BlitterFIFO.Peek() >> 24].fifo_fb_len)
        return false;

    return true;
}

static INLINE bool GPU_DMACanWrite(void) { return GPU_CalcFIFOReadyBit(); }
...
static INLINE int32 GPU_GetScanlineNum(void) { return GPU.scanline; }
