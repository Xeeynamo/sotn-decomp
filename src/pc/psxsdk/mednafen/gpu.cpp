...
extern "C" void GPU_Init(bool pal_clock_and_tv)
{
...
static void Command_FBWrite(const uint32 *cb)
{
 ...
 if(!FBRW_H)
  ...
  printf("Fbwrite to x: %d y: %d w %d h %d\n", FBRW_X, FBRW_Y, FBRW_W, FBRW_H);
...
//  printf("*******************cmdw %08X DFE: %d -- scanline=%d\n", cmdw, dfe, scanline);
}
...
MDFN_FASTCALL extern "C" void GPU_Write(const pscpu_timestamp_t timestamp, uint32 A, uint32 V)
{
 V <<= (A & 3) * 8;

//  printf("GPU_Write a %d\n", A);
...
extern "C" MDFN_FASTCALL pscpu_timestamp_t GPU_Update(const pscpu_timestamp_t sys_timestamp)
{
  DrawTimeAvail = 2048;
   ProcessFIFO();
 return 0;
 ...
 int32 sys_clocks = sys_timestamp;//sys_timestamp - lastts;
 int32 gpu_clocks;
...
int32 lw[512] = {0};
uint32 pix2[1024*1024] = {0};

extern "C" uint8* GetPix()
{
  return (uint8*)&pix2[0];
}

extern "C" uint8* GetFb()
{
  return (uint8*)&GPU.GPURAM[0][0];
}

extern "C" void GPU_StartFrameC()
{
  // spec.surface = &surf;
  // spec.surface->pixels = (uint32*)&pix2;
  spec.LineWidths = (int32*)&lw;
  GPU_StartFrame(&spec);
}
#if 0
