...
//
// It's highly unlikely the user will want these if they're intentionally
...
#define PSX_DBG_WARNING                                                        \
    1 // Warning about game doing questionable things/hitting stuff that might
      // not be emulated correctly.
#define PSX_DBG_BIOS_PRINT 2 // BIOS printf/putchar output.
#define PSX_DBG_SPARSE                                                         \
    3 // Sparse(relatively) information debug messages(CDC commands).
#define PSX_DBG_FLOOD                                                          \
    4 // Heavy informational debug messages(GPU commands; TODO).

#if PSX_DBGPRINT_ENABLE
void PSX_DBG(unsigned level, const char* format, ...) noexcept MDFN_COLD
    MDFN_FORMATSTR(gnu_printf, 2, 3);
void PSX_DBG_BIOS_PUTC(uint8 c) noexcept;

#define PSX_WARNING(format, ...)                                               \
    { PSX_DBG(PSX_DBG_WARNING, format "\n", ##__VA_ARGS__); }
...
void MDFN_FASTCALL PSX_MemWrite8(
    pscpu_timestamp_t timestamp, uint32 A, uint32 V);
void MDFN_FASTCALL PSX_MemWrite16(
    pscpu_timestamp_t timestamp, uint32 A, uint32 V);
void MDFN_FASTCALL PSX_MemWrite24(
    pscpu_timestamp_t timestamp, uint32 A, uint32 V);
void MDFN_FASTCALL PSX_MemWrite32(
    pscpu_timestamp_t timestamp, uint32 A, uint32 V);
...
// void PSX_GPULineHook(const pscpu_timestamp_t timestamp, const
// pscpu_timestamp_t line_timestamp, bool vsync, uint32 *pixels, const
// MDFN_PixelFormat* const format, const unsigned width, const unsigned
// pix_clock_offset, const unsigned pix_clock, const unsigned
// pix_clock_divider);
