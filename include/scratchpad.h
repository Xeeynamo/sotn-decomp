...
// Access to the Scratchpad memory. Different on different systems.
    ...
    // On PSX/PSP the sp2 / sp(field) macros alias the fixed hardware
    // scratchpad, shared across the servant's update functions. On PC SP(x)
    // gets expanded, providing one shared scratchpad buffer for the whole
    // translation unit.
    #define SP_LEN 0x400
