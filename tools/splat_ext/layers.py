...
class PSXSegLayers(N64Segment):
    ...
    def split(self, rom_bytes):
        ...
        self.print_splat_hints(data)

        with open(path, "w") as f:
            f.write(json.dumps(data, indent=4))

    def print_splat_hints(self, rooms):
        def has_asset_extracted(name):
            file_path = options.opts.asset_path / self.dir / name
            return os.path.exists(file_path)

        def print_individual_splat_hints(name, kind):
            if has_asset_extracted(f"{name}.{kind}.bin"):
                return
            offset = int(name[2:], 16)  # assumes name always starts with D_ or 0x
            offset = offset - (self.vram_start - self.rom_start)
            log.write(f"- [0x{offset:X}, {kind}, {name}]", status="warn")
        ...
        for layer in layers:
            print_individual_splat_hints(layer["data"], "tilelayout")
        for layer in layers:
            print_individual_splat_hints(layer["tiledef"], "tiledef")
