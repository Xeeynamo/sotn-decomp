...
func patchLine(w *bufio.Writer, line string, nLine int) error {
	...
	case "rem":
		...
		dstReg := tokens[1] + tokens[2]
		w.WriteString("\t.set\tnoreorder\n")
		w.WriteString("\t.set\tnomacro\n")
		w.WriteString("\t.set\tnoat\n")
		w.WriteString(fmt.Sprintf("\tdiv\t$0,%s,%s\n", srcReg1, srcReg2))
		w.WriteString(fmt.Sprintf("\tbnez\t%s,.L_NOT_DIV_BY_ZERO_%d\n", srcReg2, nLine))
		w.WriteString("\tnop\n")
		w.WriteString("\tbreak\t0x7\n")
		w.WriteString(fmt.Sprintf(".L_NOT_DIV_BY_ZERO_%d:\n", nLine))
		w.WriteString("\taddiu\t$1,$0,-1\n")
		w.WriteString(fmt.Sprintf("\tbne\t%s,$1,.L_DIV_BY_POSITIVE_SIGN_%d\n", srcReg2, nLine))
		w.WriteString("\tlui\t$1,0x8000\n")
		w.WriteString(fmt.Sprintf("\tbne\t%s,$1,.L_DIV_BY_POSITIVE_SIGN_%d\n", srcReg1, nLine))
		w.WriteString("\tnop\n")
		w.WriteString("\tbreak\t0x6\n")
		w.WriteString(fmt.Sprintf(".L_DIV_BY_POSITIVE_SIGN_%d:\n", nLine))
		w.WriteString(fmt.Sprintf("\tmfhi\t%s\n", dstReg))
		w.WriteString("\tnop\n")
		w.WriteString("\t.set\tat\n")
		w.WriteString("\t.set\tmacro\n")
		w.WriteString("\t.set\treorder\n")
	case "div":
		srcReg1 := tokens[4] + tokens[5]
		srcReg2 := tokens[7] + tokens[8]
		dstReg := tokens[1] + tokens[2]
