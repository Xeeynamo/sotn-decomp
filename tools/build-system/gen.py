...
def add_assets_config(nw: ninja_syntax.Writer, version: str, gen_done_path: str):
    if gen_done_path in entries:
        return
    entries[gen_done_path] = {}
    nw.build(
        rule="assets-build",
        inputs=f"config/assets.{version}.yaml",
        outputs=gen_done_path,
...
def add_prg_weapons(
    ...
    undefined_syms_auto_path: str,
):
    cpp_flags = f"-DW_{weapon_id} -DWEAPON{hand_id}"
    objs = [
        add_c(nw, version, f"{src_path}/w_{weapon_id}.c", [gen_done_path], cpp_flags),
        add_s(nw, version, f"{asm_path}/data/w_{weapon_id}.data.s", ld_path),
        add_s(nw, version, f"{asm_path}/data/w_{weapon_id}.sbss.s", ld_path),
    ]
    step_elf = f"build/{version}/weapon/w{hand_id}_{weapon_id}.elf"
    nw.build(
        rule="psx-ld",
        outputs=step_elf,
        ...
        variables={
            "version": version,
            "obj_files": objs,
            "map_out": f"build/{version}/weapon/w{hand_id}_{weapon_id}.map",
    ...
    step_bin = f"build/{version}/weapon/w{hand_id}_{weapon_id}.bin"
    nw.build(
...
def add_splat_config(nw: ninja_syntax.Writer, version: str, file_name: str):
    ...
    ld_script_path = str(splat_config["options"]["ld_script_path"])
    ...
    if src_path == "src/st":
        ...
        gen_done_path = os.path.join(
            src_path, ovl_name.replace("st", ""), "gen", f"done_{version}"
        )
    nw.build(
        rule="splat",
    ...
    for segment in splat_config["segments"]:
        ...
        if segment["type"] == "data":
            ...
            objs.append(add_s(nw, version, asm_name, ld_script_path))
            continue
        if segment["type"] == "grey4":
            weapon_gfx = segment["name"]
            weapons.append(
                add_gfx_weapon(
                    nw, version, os.path.join(asset_path, weapon_gfx), ld_script_path
                )
            )
            continue
        if is_weapon(ovl_name):
            weapon_id = weapon_gfx[2:]
        if is_weapon(ovl_name) and segment["type"] == "code":
            if not weapon_id:
                raise Exception("weapon_id not assigned")
            weapons.append(
                add_prg_weapons(
                    nw,
                    version,
        ...
        for subsegment in segment["subsegments"]:
            ...
            if kind == "c" or kind == ".data" or kind == ".rodata" or kind == ".bss":
                objs.append(
                    add_c(nw, version, f"{src_path}/{name}.c", [gen_done_path], "")
                )
                ...
                objs.append(add_s(nw, version, f"{asm_path}/{name}.s", ld_script_path))
    ...
    if is_weapon(ovl_name):
        nw.build(
            rule="concat",
            outputs=f"build/{version}/WEAPON0.BIN",
            inputs=weapons,
        )
    else:
...
def add_checksum(nw: ninja_syntax.Writer, version: str, file_name: str):
    ...
    binaries = [binary for binary in binaries if not "main" in binary]
    if version == "us":
