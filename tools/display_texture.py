...
def convert_rgb555(in_array):
    ...
    for in_row in in_array:
        ...
        for i in range(0, len(in_row), 2):
            ...
            pixel = [r * 8, g * 8, b * 8] # 5 bits is 0-31, multiply to 0-255
...
def load_global_cluts(full_image):
    ...
    for idx in range(256):
        ...
        clut = clut_zone[row_number : row_number+1, col_number * 16 : (col_number+1) * 16]
...
def get_tpage_by_number(raw_dump, tpage_num):
    ...
    tpage_left = (tpage_num % 16) * 128 # Modulo 16 gets the horizonal coordinate, then they are 128 pixels wide.
...
parser.add_argument('tpage_num', type=lambda x: int(x, 0))
parser.add_argument('clut_num', type=lambda x: int(x, 0))
parser.add_argument("UV_vals", nargs='*', type=lambda x: int(x, 0), help="Additional numbers or 'WHOLE'")

# Alternate run modes
parser.add_argument("--whole", action="store_true", help="Show the whole tpage+clut")
parser.add_argument("--showclut", action="store_true", help="Just show the 16 colors in the CLUT")
