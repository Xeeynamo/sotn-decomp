...
DUPS_THRESHOLD  ?= .90
DUPS			:= cd $(TOOLS_DIR)/dups; cargo run --release -- --threshold $(DUPS_THRESHOLD) --splat-config-glob '../../$(CONFIG_DIR)/splat.$(VERSION).*.yaml' --output-file ../../$(REPORTS_DIR)/duplicates.txt
