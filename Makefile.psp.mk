...
$(BUILD_DIR)/%.c.o: %.c bin/mwccpsp.exe
	mkdir -p $(dir $@)
	$(MWCPP_APP) $< -o $@.post.c
	$(CCPSP) -gccinc -Iinclude -D_internal_version_$(VERSION) -O0 -c -lang c -sdatathreshold 0 -o $@ $@.post.c
	rm $@.post.c
$(BUILD_DIR)/asm/psp%.s.o: asm/psp%.s
	mkdir -p $(dir $@)
	$(ASPSP) -o $@ $<


$(BUILD_DIR)/assets/servant/tt_000/header.bin.o: assets/servant/tt_000/header.bin
	mkdir -p $(dir $@)
...
$(BUILD_DIR)/tt_%.ld: $(CONFIG_DIR)/splat.pspeu.tt_%.yaml $(PSX_BASE_SYMS) $(CONFIG_DIR)/symbols.pspeu.tt_%.txt
	$(SPLAT_PIP) $<
$(BUILD_DIR)/tt_%.elf: $(BUILD_DIR)/tt_%.ld $$(call list_o_files_psp,servant/tt_$$*)
	$(call link,tt_$*,$@)

# cannot remove it for some reason? makefile bug?
_ignoreme_tt_000: $(BUILD_DIR)/src/servant/tt_000_psp/80.c.o $(BUILD_DIR)/asm/pspeu/servant/tt_000/data/4C80.data.s.o $(BUILD_DIR)/asm/pspeu/servant/tt_000/data/5E00.rodata.s.o
