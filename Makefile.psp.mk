...
OPT_HI_FUNCS	:= $(addsuffix .c.o,80 1E50 33F0 A710 C0B0 EC60 F4D0 186E8 61F30 624DC 628AC 63C90 64EE0) # These objects will get -O4,p instead of -Op
COMPILER		 = $(SOTNSTR) -f $< | $(PYTHON) $(MWCCGAP_APP)
...
$(addprefix $(BUILD_DIR:$(VERSION)=pspeu)/,$(addsuffix .elf,$(filter-out main,$(GAME)))): $(BUILD_DIR)/%.elf: $(BUILD_DIR)/%.ld $$(call get_functions,%) $$(call new_list_o_files,%_psp) $$(if $$(filter-out dra,%),$(BUILD_DIR)/assets/%/mwo_header.bin.o)
	$(call new_link,$*,$@)
$(BUILD_DIR:$(VERSION)=pspeu)/st%.elf: $(BUILD_DIR)/st%.ld $$(call get_functions,%,st) $$(call new_list_o_files,st/%_psp) $(BUILD_DIR)/assets/st/%/mwo_header.bin.o
	$(call new_link,st$*,$@)
$(BUILD_DIR:$(VERSION)=pspeu)/bo%.elf: $(BUILD_DIR)/bo%.ld $$(call get_functions,%,bo) $$(call new_list_o_files,boss/%_psp) $(BUILD_DIR)/assets/boss/%/mwo_header.bin.o
	$(call new_link,st$*,$@)
$(BUILD_DIR:$(VERSION)=pspeu)/tt_%.elf: $(BUILD_DIR)/tt_%.ld $$(call new_list_o_files,servant/tt_$$*) $(BUILD_DIR)/assets/servant/tt_%/mwo_header.bin.o
	$(call new_link,tt_$*,$@)
