...
COMPILER		 = $(SOTNSTR_APP) process -p -f $< | $(PYTHON) $(MWCCGAP_APP)
COMPILER_REQS	:= $(MWCCPSP) $(MWCCGAP_APP) $(AS) | $(VENV_DIR)/bin
...
# Step 1/2 of extract
$(BUILD_DIR)/%.ld assets/%/mwo_header.bin: $(CONFIG_DIR)/splat.$(VERSION).%.yaml $(BASE_SYMBOLS) $(CONFIG_DIR)/symbols.$(VERSION).%.txt | $(EXTRACTED_DISK_DIR) $(VENV_DIR)/bin
	$(muffle)$(SPLAT) $<
