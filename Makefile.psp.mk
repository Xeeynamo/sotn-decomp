...
ric_psp: $(PSP_BUILD_DIR)/ric.bin

$(PSP_BUILD_DIR)/%.bin: $(PSP_BUILD_DIR)/%.elf
	$(OBJCOPY) -O binary $< $@
$(PSP_BUILD_DIR)/ric.ld: $(CONFIG_DIR)/splat.pspeu.ric.yaml $(PSX_BASE_SYMS) $(CONFIG_DIR)/symbols.pspeu.ric.txt
	$(SPLAT_PIP) $<
$(PSP_BUILD_DIR)/tt_%.ld: $(CONFIG_DIR)/splat.pspeu.tt_%.yaml $(PSX_BASE_SYMS) $(CONFIG_DIR)/symbols.pspeu.tt_%.txt
	$(SPLAT_PIP) $<
$(PSP_BUILD_DIR)/ric.elf: $(PSP_BUILD_DIR)/ric.ld src/ric_psp/80.c
	$(call link,ric_psp,$@)
